...
import com.example.yuehaoting.lyrics.utils.ColorUtils;
import com.example.yuehaoting.lyrics.utils.LyricsUtils;
...
public class ManyLyricsView extends AbstractLrcView {
  ...
  private void drawManyLrcView(Canvas canvas) {
    ...
    //获取中间位置
    mCentreY =
        (getHeight() + LyricsUtils.getTextHeight(paintHL)) * 0.5f + getLineAtHeightY(lyricsLineNum)
            - mOffsetY;
    ...
    float lineBottomY = drawDownLyrics(canvas, paint, paintHL, splitLyricsLineInfos,
        splitLyricsLineNum, splitLyricsWordIndex, spaceLineHeight, lyricsWordHLTime, mCentreY);

    //未超出下视图
    if (lineBottomY + spaceLineHeight < getHeight()) {
      //画额外歌词
      lineBottomY = drawDownExtraLyrics(canvas, extraLrcPaint, extraLrcPaintHL, lyricsLineNum,
          extraSplitLyricsLineNum, extraSplitLyricsWordIndex, extraLrcSpaceLineHeight,
    ...
    for (int i = lyricsLineNum + 1; i < lrcLineInfos.size(); i++) {
      ...
      lineBottomY = drawDownLyrics(canvas, paint, paintHL, lyricsLineInfos, -1, -2, spaceLineHeight,
          -1, lineBottomY);
      ...
      //画额外歌词
      lineBottomY = drawDownExtraLyrics(canvas, extraLrcPaint, extraLrcPaintHL, i, -1, -2,
          extraLrcSpaceLineHeight, -1, -1, lineBottomY);
    }
    ...
    for (int i = lyricsLineNum - 1; i >= 0; i--) {
      ...
      lineTopY = drawUpExtraLyrics(canvas, extraLrcPaint, lyricsLineInfos, i,
          extraLrcSpaceLineHeight, lineTopY);
    }
   ...
   * @return
   */
  private float drawDownLyrics(Canvas canvas, Paint paint, Paint paintHL,
      List<LyricsLineInfo> splitLyricsLineInfos, int splitLyricsLineNum, int splitLyricsWordIndex,
      float spaceLineHeight, float lyricsWordHLTime, float fristLineTextY) {
    //获取数据
...
//            }
      ...
      if (lineBottomY < mShadeHeight) {
        alpha = mMaxAlpha - (int) ((mShadeHeight - lineBottomY) * (mMaxAlpha - mMinAlpha)
            / mShadeHeight);
      } else if (lineBottomY > getHeight() - mShadeHeight) {
        alpha = mMaxAlpha - (int) (
            (lineBottomY - (getHeight() - mShadeHeight)) * (mMaxAlpha - mMinAlpha) / mShadeHeight);
      }
      ...
      if (i < curLyricsLineNum) {
        ...
        //绘画动感歌词
        float lineLyricsHLWidth = LyricsUtils.getLineLyricsHLWidth(lyricsReader.getLyricsType(),
            paint, splitLyricsLineInfos.get(i), splitLyricsWordIndex, lyricsWordHLTime);
        LyricsUtils.drawDynamicText(canvas, paint, paintHL, paintColors, paintHLColors, text,
            lineLyricsHLWidth, textX, lineBottomY);
...
//            canvas.drawLine(0, lineBottomY, 720, lineBottomY, paint);
   ...
   * @return
   */
  private float drawDownExtraLyrics(Canvas canvas, Paint paint, Paint paintHL, int lyricsLineNum,
      int extraSplitLyricsLineNum, int extraSplitLyricsWordIndex, float extraLrcSpaceLineHeight,
      float lyricsWordHLTime, float translateLyricsWordHLTime, float lineBottomY) {
    //获取数据
    LyricsReader lyricsReader = getLyricsReader();
    ...
    if (extraLrcStatus == AbstractLrcView.EXTRALRCSTATUS_SHOWTRANSLATELRC) {
      ...
      if (translateLrcLineInfos != null && translateLrcLineInfos.size() > 0) {
        //以动感歌词的形式显示翻译歌词
        List<LyricsLineInfo> translateSplitLyricsLineInfos = translateLrcLineInfos.get(
            lyricsLineNum).getSplitLyricsLineInfos();
        lineBottomY += extraLrcSpaceLineHeight - spaceLineHeight;
        if (lyricsReader.getLyricsType() == LyricsInfo.DYNAMIC
            && extraLrcStatus == AbstractLrcView.EXTRALRCSTATUS_SHOWTRANSLATELRC
            && translateDrawType == AbstractLrcView.TRANSLATE_DRAW_TYPE_DYNAMIC) {
          lineBottomY = drawDownLyrics(canvas, paint, paintHL, translateSplitLyricsLineInfos,
              extraSplitLyricsLineNum, extraSplitLyricsWordIndex, extraLrcSpaceLineHeight,
              translateLyricsWordHLTime, lineBottomY);
        } else {
          //画lrc歌词
          ...
          lineBottomY = drawDownLyrics(canvas, paint, paintHL, translateSplitLyricsLineInfos,
              splitLyricsLineNum, -2, extraLrcSpaceLineHeight, -1, lineBottomY);
        }
      ...
      if (transliterationLrcLineInfos != null && transliterationLrcLineInfos.size() > 0) {
        //获取分割后的音译歌词行
        List<LyricsLineInfo> transliterationSplitLrcLineInfos = transliterationLrcLineInfos.get(
            lyricsLineNum).getSplitLyricsLineInfos();
        lineBottomY += extraLrcSpaceLineHeight - spaceLineHeight;
        lineBottomY = drawDownLyrics(canvas, paint, paintHL, transliterationSplitLrcLineInfos,
            extraSplitLyricsLineNum, extraSplitLyricsWordIndex, extraLrcSpaceLineHeight,
            lyricsWordHLTime, lineBottomY);
        lineBottomY += spaceLineHeight - extraLrcSpaceLineHeight;
      }
    }
    return lineBottomY;
   ...
   */
  private float drawUpExtraLyrics(Canvas canvas, Paint paint,
      List<LyricsLineInfo> splitLyricsLineInfos, int lyricsLineNum, float extraLrcSpaceLineHeight,
      float lineTopY) {
    //获取数据
    int extraLrcStatus = getExtraLrcStatus();
    float spaceLineHeight = getSpaceLineHeight();
    ...
    if (extraLrcStatus == AbstractLrcView.EXTRALRCSTATUS_SHOWTRANSLATELRC) {
      ...
      if (translateLrcLineInfos != null && translateLrcLineInfos.size() > 0) {
        ...
        lineTopY = drawUpLyrics(canvas, paint, translateSplitLyricsLineInfos,
            extraLrcSpaceLineHeight, lineTopY);
        if (lineTopY < 0) {
          return lineTopY;
        }
      ...
      if (transliterationLrcLineInfos != null && transliterationLrcLineInfos.size() > 0) {
        ...
        List<LyricsLineInfo> transliterationSplitLrcLineInfos = transliterationLrcLineInfos.get(
            lyricsLineNum).getSplitLyricsLineInfos();
        lineTopY -= (LyricsUtils.getTextHeight(paint) + spaceLineHeight);
        lineTopY = drawUpLyrics(canvas, paint, transliterationSplitLrcLineInfos,
            extraLrcSpaceLineHeight, lineTopY);
   ...
   * @return
   */
  private float drawUpLyrics(Canvas canvas, Paint paint, List<LyricsLineInfo> splitLyricsLineInfos,
      float spaceLineHeight, float fristLineTextY) {
    int[] paintColors = getPaintColors();
...
////            }
      ...
      if (lineTopY < mShadeHeight) {
        alpha =
            mMaxAlpha - (int) ((mShadeHeight - lineTopY) * (mMaxAlpha - mMinAlpha) / mShadeHeight);
      } else if (lineTopY > getHeight() - mShadeHeight) {
        alpha =
            mMaxAlpha - (int) ((lineTopY - (getHeight() - mShadeHeight)) * (mMaxAlpha - mMinAlpha)
                / mShadeHeight);
      }
...
//            canvas.drawLine(0, lineTopY, 720, lineTopY, paint);
  ...
  private void drawIndicator(Canvas canvas, int startTime) {
    ...
    LinearGradient linearGradientHL = new LinearGradient(lineLeft, lineY + lineH, lineR,
        lineY + lineH, new int[]{
        ColorUtils.parserColor(mPaintLineColor, 255), ColorUtils.parserColor(mPaintLineColor, 0),
        ColorUtils.parserColor(mPaintLineColor, 0), ColorUtils.parserColor(mPaintLineColor, 255)},
  ...
  private void updateManyLrcView(long playProgress) {
    ...
    //
    int newLyricsLineNum = LyricsUtils.getLineNumber(lyricsReader.getLyricsType(), lrcLineInfos,
        playProgress, lyricsReader.getPlayOffset());
  ...
  public void setOnClickListenerL(Monitor mMonitor){
  ...
  public boolean onTouchEvent(MotionEvent event) {
    ...
    switch (actionId) {
      case MotionEvent.ACTION_DOWN:
        ...
        Timber.tag("观察xy轴").v("按下x和y的位置 %s %s", mx, my);
        //发送还原
        mHandler.removeMessages(RESETLRCVIEW);

        Timber.v("ACTION_DOWN%s %s", mPlayBtnRect != null, isPlayClick(event));
      ...
      case MotionEvent.ACTION_UP:
       ...
       if (mx==nx&& my==ny  &&!isInPlayBtnRect ){
       mMonitor.setOnClickListener();
       }else {



        //判断是否在滑动和是否点击了播放按钮
        Timber.v("setOnClickListener %s", isInPlayBtnRect);
        if (isInPlayBtnRect) {
          Timber.v("setOnClickListener1 %s", "在点击");
          mHandler.removeMessages(RESETLRCVIEW);
          ...
          Timber.v("setOnClickListener在滑动");
        }
       }
  ...
  private int getLineSizeNum(int lyricsLineNum) {
    ...
    if (extraLrcStatus == AbstractLrcView.EXTRALRCSTATUS_SHOWTRANSLATELRC) {
      if (translateLrcLineInfos != null && translateLrcLineInfos.size() > 0) {
        List<LyricsLineInfo> tempTranslateLrcLineInfos = translateLrcLineInfos.get(lyricsLineNum)
            .getSplitLyricsLineInfos();
        lineSizeNum += tempTranslateLrcLineInfos.size();
      }
    } else if (extraLrcStatus == AbstractLrcView.EXTRALRCSTATUS_SHOWTRANSLITERATIONLRC) {
      if (transliterationLrcLineInfos != null && transliterationLrcLineInfos.size() > 0) {
        List<LyricsLineInfo> tempTransliterationLrcLineInfos = transliterationLrcLineInfos.get(
            lyricsLineNum).getSplitLyricsLineInfos();
  ...
  private int getLineAtHeightY(int lyricsLineNum) {
    ...
    for (int i = 0; i < lyricsLineNum; i++) {
      ...
      List<LyricsLineInfo> lyricsLineInfos = lyricsLineInfo.getSplitLyricsLineInfos();
      lineAtHeightY +=
          (LyricsUtils.getTextHeight(paint) + spaceLineHeight) * lyricsLineInfos.size();

      //判断是否有翻译歌词或者音译歌词
      if (extraLrcStatus == AbstractLrcView.EXTRALRCSTATUS_SHOWTRANSLATELRC) {
        if (translateLrcLineInfos != null && translateLrcLineInfos.size() > 0) {
          List<LyricsLineInfo> tempTranslateLrcLineInfos = translateLrcLineInfos.get(i)
              .getSplitLyricsLineInfos();
          lineAtHeightY += (LyricsUtils.getTextHeight(extraLrcPaint) + extraLrcSpaceLineHeight)
              * tempTranslateLrcLineInfos.size();
        }
      } else if (extraLrcStatus == AbstractLrcView.EXTRALRCSTATUS_SHOWTRANSLITERATIONLRC) {
        if (transliterationLrcLineInfos != null && transliterationLrcLineInfos.size() > 0) {
          List<LyricsLineInfo> tempTransliterationLrcLineInfos = transliterationLrcLineInfos.get(i)
              .getSplitLyricsLineInfos();
          lineAtHeightY += (LyricsUtils.getTextHeight(extraLrcPaint) + extraLrcSpaceLineHeight)
              * tempTransliterationLrcLineInfos.size();
        }
      }
    }
    ...
    for (int i = 0; i < lrcLineInfos.size(); i++) {
      ...
      if (extraLrcStatus == AbstractLrcView.EXTRALRCSTATUS_SHOWTRANSLATELRC) {
        if (translateLrcLineInfos != null && translateLrcLineInfos.size() > 0) {
          List<LyricsLineInfo> tempTranslateLrcLineInfos = translateLrcLineInfos.get(i)
              .getSplitLyricsLineInfos();
          lineHeight += (LyricsUtils.getTextHeight(extraLrcPaint) + extraLrcSpaceLineHeight)
              * tempTranslateLrcLineInfos.size();
        }
      } else if (extraLrcStatus == AbstractLrcView.EXTRALRCSTATUS_SHOWTRANSLITERATIONLRC) {
        if (transliterationLrcLineInfos != null && transliterationLrcLineInfos.size() > 0) {
  ...
  /**
   * //字体大小、额外歌词显示或者空行大小改变，则对歌词的位置进行修改 重置scroller的finaly
   */
  ...
  public void setLyricsReader(LyricsReader lyricsReader) {
    ...
    if (lyricsReader != null && lyricsReader.getLyricsType() == LyricsInfo.DYNAMIC) {
      ...
      //翻译歌词以动感歌词形式显示
      if (extraLrcType == AbstractLrcView.EXTRALRCTYPE_BOTH
          || extraLrcType == AbstractLrcView.EXTRALRCTYPE_TRANSLATELRC) {
