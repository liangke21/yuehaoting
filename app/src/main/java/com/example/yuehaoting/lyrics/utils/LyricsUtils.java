...
/**
 * 歌词处理类 Created by zhangliangming on 2018-02-23.
 */
...
public class LyricsUtils {
   ...
   * @param paintHLColor
   */
  public static void drawDynamiLyrics(Canvas canvas, int lyricsType, Paint paint, Paint paintHL, Paint paintOutline, LyricsLineInfo lyricsLineInfo, float lyricsLineHLWidth, int viewWidth,
      int lyricsWordIndex, float lyricsWordHLTime, float textY, float paddingLeftOrRight, int[] paintColor, int[] paintHLColor) {
    //获取行歌词高亮宽度
  ...
  public static int getLineNumber(int lyricsType, TreeMap<Integer, LyricsLineInfo> lyricsLineTreeMap, long curPlayingTime, long playOffset) {
    try {
      ...
      if (lyricsType == LyricsInfo.LRC) {
        //  Log.e("歌词列表", String.valueOf(lyricsLineTreeMap.size()));
        //lrc歌词
...
//////////////////////////////////分割歌词///////////////////////////////////////////
  ...
  public static long getLrcLenTime(TreeMap<Integer, LyricsLineInfo> lyricsLineTreeMap, int lyricsLineNum, long oldPlayingTime, long playOffset) {
    ...
    long curPlayingTime = oldPlayingTime + playOffset;
    LyricsLineInfo lyrLine = lyricsLineTreeMap.get(lyricsLineNum);
    if (curPlayingTime < lyrLine.getStartTime()) {
      return 0;
    }
